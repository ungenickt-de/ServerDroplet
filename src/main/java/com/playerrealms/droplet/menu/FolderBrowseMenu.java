...
public class FolderBrowseMenu extends ChestPacketMenu {
	...
	public FolderBrowseMenu(File rootFolder, String path, Player player) {
	...
	public FolderBrowseMenu(File rootFolder, Player player) {
					...
					ap.setHandler(new AnvilPacketMenuHandler() {
						...
						public void onResult(String text, Player pl) {
							...
							try {
								...
								FolderBrowseMenu fbm = new FolderBrowseMenu(rootFolder, path, player);
								fbm.open(pl);
							} catch (IOException e) {
								e.printStackTrace();
							}
	...
	public List<File> getFiles() {
		List<File> files = new ArrayList<>();
		for(File file : getPath().listFiles()) {
			if(!(file.getName().endsWith(".yml") || file.getName().endsWith(".txt") || file.getName().endsWith(".json"))) {
				continue;
			}
			if(!file.isDirectory()) {
				files.add(file);
